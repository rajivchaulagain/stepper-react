...
const data = {
  ...
  mockResults : [
    {
      id: 1,
      title: "Athlone Community Training Center (CTC) - Catering and Hospitality",
...
const SelectCourseCard = ({ title, onClick, isSelected, id , showCheckbox=true }) => {
...
const CourseFormCard = (props) => {
  ...
  return (
    <div className="course-form box-shadow">
      ...
      <input type="text" placeholder="Search course..." value={props.inputSearch} onChange={(e) => props.handleSearch(e.target.value)} />
      ...
      <div>
        ...
        <div className="select-wrapper">
          ...
          <option disabled selected value="" onChange={() => props.handleCourseTypeSelection(props.courseType.title)}>Course Type</option>
          {data.courseType.map((type) => (<option key={type.id} value={type.title}>{type.title}</option>))}
          </select>
          <select>
          <option disabled selected value="">Location</option>
          {data.location.map((loc) => (<option key={loc.id} value={loc.title}>{loc.title}</option>))}
          </select>
            {/* <Select
            options={options}
          ...
          /> */}
          <select>
            <option disabled selected value="">Areas of Interest</option>
            {data.interestArea.map((area) => (<option key={area.id} value={area.title}>{area.title}</option>))}
          </select>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="showing-results">
      <div className="card-body box-shadow">
        <div>
          <h3><span className="text-primary">TU869</span>{searchVaue.title}</h3>
          <hr />
        </div>
        <div>
          <ul>
...
const Stepper = () => {
  ...
  const handleInterestAreaSelection = (interestArea) => {
    if (selectedInterestArea.includes(interestArea)) {
      setSelectedInterestArea(selectedInterestArea.filter((area) => area !== interestArea));
    } else {
  ...
  return (
    <div className="stepper">
      ...
      <div className="stepper-body">
        {currentStep === 1 && (
          <div className="step">
            <button disabled={!selectedCourseType} onClick={handleNext} style={{float : 'right'}}>
              Next
            </button>
            <SectionTitle
        ...
        {currentStep === 2 && (
          <div className="step">
              <button disabled={!selectedInterestArea} onClick={handleNext} style={{float : 'right'}}>
                Next
              </button>
              <button onClick={handlePrevious} style={{float : 'left'}}>Previous</button>
              <SectionTitle
              title="Interest Area"
              paragraph="We offer a wide range of full and part time courses cross Longford and Westmeath. Full time courses take place during the day and part time course are more flexible, courses are offered during the day, in the evening or online. Courses will be filtered according to your chosen option."
            />
